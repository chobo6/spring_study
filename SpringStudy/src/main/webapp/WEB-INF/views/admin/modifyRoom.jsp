...
<body>
	...
	<form action="" method="post">
		...
		<select name="viewType">
			<option value="OCN" <c:if test="${room.viewType eq 'OCN'}">selected</c:if>>오션뷰</option>
			<option value="CTY" <c:if test="${room.viewType eq 'CTY'}">selected</c:if>>시티뷰</option>
			<option value="MOT" <c:if test="${room.viewType eq 'MOT'}">selected</c:if>>마운틴뷰</option>
		</select>
